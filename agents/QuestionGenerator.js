...
class QuestionGenerator {
  ...
  _mathBudgetTemplates() {
    return ['beginner', 'intermediate', 'advanced'].map(level => ({
      ...
      generator: () => {
        const depts = ['총무팀', '기획팀', '사업팀', '홍보팀', '인사팀', '재무팀', 'IT팀', '법무팀', '감사팀', '교육팀'];
        this._shuffle(depts);
  ...
  _mathGrowthTemplates() {
    return ['intermediate', 'advanced'].map(level => ({
      ...
      generator: () => {
        const contexts = [
          { item: '매출액', unit: '억원', org: 'A공사' },
          { item: '민원처리 건수', unit: '건', org: 'B공단' },
          { item: '이용자 수', unit: '명', org: 'C기관' },
          { item: '수출액', unit: '백만달러', org: 'D공사' },
          { item: '예산 집행액', unit: '억원', org: 'E기관' },
          { item: '신규 고용 인원', unit: '명', org: 'F공단' },
        ];
        const ctx = contexts[this._rand(0, contexts.length - 1)];
  ...
  _mathProportionTemplates() {
    return ['beginner', 'intermediate'].map(level => ({
      ...
      generator: () => {
        const catSets = [
          ['인건비', '운영비', '사업비', '출장비', '교육비'],
          ['급여', '복리후생비', '여비', '업무추진비', '시설비'],
          ['국내출장비', '해외출장비', '교통비', '숙박비', '식비'],
          ['정규직', '계약직', '파견직', '인턴', '용역']
        ];
        const categories = catSets[this._rand(0, catSets.length - 1)];
        ...
        return {
          ...
          question: `다음은 기관의 항목별 지출 내역이다. (단위: 만원)\n\n${categories.map((c, i) => `${c}: ${values[i]}`).join(', ')}\n합계: ${total}\n\n다음 중 옳은 것은?`,
      ...
      generator: () => {
        const regTypes = [
          () => {
            ...
            const dept = ['총무과', '기획과', '사업과', '인사과'][this._rand(0, 3)];
      ...
      generator: () => {
        const scenarios = [
          () => {
            ...
            const correct = `${exception}은 이번 ${event} 대상에서 제외된다.`;
            const wrongs = [
            ...
            return {
              ...
              choices, answer: choices.indexOf(correct),
              explanation: `${exception}은 제외, 기간 ${month}월 ${endDay}일까지, 미이수 시 ${penalty}.`
            };
          },
          () => {
      ...
      generator: () => {
        const scenarios = [
          { situation: '민원처리 지연', criteria: ['긴급도', '영향범위', '해결가능성', '비용'],
            ...
            wrongs: ['접수 순서대로 무조건 처리한다.', '비용이 가장 적게 드는 민원부터 처리한다.', '해결이 쉬운 민원부터 처리하여 건수를 늘린다.'] },
          { situation: '시스템 장애 발생', criteria: ['복구시간', '사용자수', '대체수단', '데이터손실위험'],
            correct: '사용자수와 데이터손실위험이 높은 시스템을 최우선 복구한다.',
            wrongs: ['모든 시스템을 동시에 복구한다.', '복구시간이 짧은 것부터 처리한다.', '대체수단이 있는 시스템은 무기한 보류한다.'] },
          { situation: '예산 삭감 대응', criteria: ['필수도', '법적의무', '성과기여도', '이해관계자영향'],
            correct: '법적 의무 사업은 유지하고, 성과기여도가 낮은 사업부터 조정한다.',
            wrongs: ['모든 사업을 균등 삭감한다.', '인건비를 최우선 삭감한다.', '성과가 높은 사업부터 삭감하여 형평성을 맞춘다.'] },
          { situation: '고객 불만 급증', criteria: ['불만원인', '재발가능성', '브랜드영향', '대응비용'],
            correct: '불만 원인을 분석하여 재발 가능성이 높고 브랜드 영향이 큰 문제부터 해결한다.',
            wrongs: ['불만 고객에게 일괄 보상금을 지급한다.', '불만 건수가 적은 유형부터 처리한다.', '고객 불만은 일시적 현상이므로 관망한다.'] },
          { situation: '인력 부족 상황', criteria: ['업무긴급도', '전문성요구', '외주가능성', '법정기한'],
            correct: '법정기한이 있는 업무를 최우선 배정하고, 외주 가능한 업무는 외부 위탁을 검토한다.',
            wrongs: ['모든 직원에게 야근을 지시한다.', '업무를 균등 분배하여 공평성을 확보한다.', '신규 채용이 완료될 때까지 업무를 보류한다.'] },
          { situation: '정보 유출 사고', criteria: ['유출범위', '정보민감도', '법적책임', '이해관계자통보'],
            correct: '유출 경로를 즉시 차단하고, 관련 법령에 따라 감독기관과 피해자에게 통보한다.',
            wrongs: ['내부 조사가 완료된 후 감독기관에 통보한다.', '유출 사실을 비공개로 처리하여 기관 이미지를 보호한다.', '법무팀의 검토가 끝날 때까지 어떠한 조치도 취하지 않는다.'] }
        ];
        const s = scenarios[this._rand(0, scenarios.length - 1)];
        const choices = [s.correct, ...s.wrongs]; this._shuffle(choices);
        return {
          area: '문제해결능력', subArea: '우선순위결정', level, type: 'PSAT형', timeLimit: 100,
          question: `기관에서 '${s.situation}' 상황이 발생하였다. 고려 기준: ${s.criteria.join(', ')}.\n\n가장 적절한 대응 방안은?`,
  ...
  _resourceTemplates() {
    return ['beginner', 'intermediate', 'advanced'].map(level => ({
      ...
      generator: () => {
        ...
        const itemSets = [
          ['인건비', '시설비', '운영비', '사업비'],
          ['급여', '교육훈련비', '시설유지비', '사업추진비'],
          ['정규직인건비', '비정규직인건비', '업무추진비', '여비교통비']
        ];
        const items = itemSets[this._rand(0, itemSets.length - 1)];
      ...
      generator: () => {
        const scenarios = [
          () => {
            ...
            const mostEfficient = projects[perPerson.indexOf(Math.min(...perPerson.map(Number)))];
        ...
        const pools = {
          beginner: [
            { q: '다음 중 컴퓨터 바이러스 감염을 예방하는 방법으로 가장 적절한 것은?',
              ...
              wrongs: ['컴퓨터를 매일 재부팅한다.', '인터넷 사용 시간을 줄인다.', '모니터 해상도를 낮춘다.'], sub: '정보보안' },
          ...
          intermediate: [
            ...
            { q: 'VLOOKUP 함수의 설명으로 옳은 것은?',
              ...
              wrongs: ['셀 범위의 합계를 계산한다.', '조건에 맞는 셀 개수를 센다.', '텍스트를 숫자로 변환한다.'], sub: '컴퓨터활용' },
  ...
  _jobTemplates() {
    const allConcepts = [
      ...
      { q: '신공공관리론(NPM)의 핵심 원리와 거리가 먼 것은?',
        ...
        wrongs: ['성과 중심 관리', '시장 메커니즘 활용', '고객 지향적 서비스'], sub: '행정이론', level: 'advanced' },
      { q: '주민참여예산제도에 대한 설명으로 옳은 것은?',
        correct: '주민이 예산 편성 과정에 직접 참여하여 의견을 제시할 수 있다.',
        wrongs: ['주민이 예산 집행을 직접 감사한다.', '주민이 세율을 직접 결정한다.', '주민이 공무원 인사에 개입한다.'], sub: '재무행정', level: 'beginner' },
      { q: '행정의 책임성 유형 중 외부적·공식적 책임성에 해당하는 것은?',
        correct: '입법부에 의한 통제',
        wrongs: ['전문가적 양심에 의한 자율 통제', '동료 집단에 의한 평가', '직업윤리에 기반한 자기 규율'], sub: '행정이론', level: 'advanced' },
      // 회계학
      { q: '복식부기의 원리에 대한 설명으로 옳은 것은?',
        correct: '모든 거래는 차변과 대변에 동시에 같은 금액으로 기록한다.',
        wrongs: ['수입만 기록하고 지출은 기록하지 않는다.', '현금 거래만 기록한다.', '매월 1회만 기록한다.'], sub: '회계원리', level: 'beginner' },
        ...
        const pools = {
          ...
          intermediate: [
            ...
            { q: '부당한 업무 지시를 받았을 때 가장 적절한 대응은?',
              ...
              wrongs: ['상급자의 지시이므로 무조건 따른다.', '동료들과 함께 집단으로 거부한다.', '언론에 제보하여 공론화한다.'] }
          ],
          advanced: [
        ...
        const pools = {
          ...
          intermediate: [
            { q: '비형식학습(Non-formal Learning)에 해당하는 것은?',
              ...
              wrongs: ['대학교 정규 교과과정 수료', '우연한 경험을 통한 학습', '국가자격시험 응시'] },
